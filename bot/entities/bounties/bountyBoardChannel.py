...
class BountyBoardChannel(SerializesToSchema[TSchema]):
    ...
    async def makeBountyEmbed(self, bounty: bounty.Bounty) -> Embed:
        ...
        infoStr = f"**Difficulty:** {bounty.techLevel}\n" \
                + f"**Reward Pool:** {lib.stringUtil.commaSplitNum(bounty.reward)} Credits\n" \
    ...
    async def loadMessageWithRetry(self, id: int, meta: str, logUrls: bool = True, initializing: bool = False) -> Optional[Message]:
        ...
        return await lib.discordUtil.discordOperationWithRetry(self.channel.fetch_message, "load message", LogCategory.bountyBoards,
                                                            "BBC", meta, id)
    ...
    async def sendMessageWithRetry(self, meta: str, *args, **kwargs) -> Optional[Message]:
        ...
        return await lib.discordUtil.discordOperationWithRetry(self.channel.send, "send message", LogCategory.bountyBoards,
                                                            "BBC", meta, *args, **kwargs)
    ...
    async def editMessageWithRetry(self, message: Message, meta: str, logUrls: bool = True, *args, **kwargs) -> \
        ...
        await lib.discordUtil.discordOperationWithRetry(message.edit, "edit message", LogCategory.bountyBoards,
                                                        "BBC", meta, *args, **kwargs)
        return message
