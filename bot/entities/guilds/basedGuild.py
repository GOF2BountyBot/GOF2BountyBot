...
from ...repositories.bountyRepository import BountyRepository, nameForDivision, divisionNameForLevel, SerializedBountyDB
from ...userAlerts import userAlerts
from ...cfg import cfg, bbData
from ...gameObjects.bounties import bountyConfig
from ..bounties import bountyDivision
from ..bounties import bountyBoardChannel
from ...gameObjects.items.gameItem import GameItem
from ..user import basedUser
...
class BasedGuild(SerializesToSchema[SerializedBasedGuildUnion]):
    ...
    def __init__(self, id: int, dcGuild: Guild, bounties: BountyRepository, commandPrefix: str = cfg.defaultCommandPrefix,
    ...
    def enableBounties(self):
        ...
        self.bountiesDB = BountyRepository(self)
    ...
    def serialize(self, **kwargs) -> SerializedBasedGuildUnion:
        ...
        if not self.bountiesDisabled:
            ...
            # Casting here because bountiesDB cannot be None if bountiesDisabled is False
            data["bountiesDB"] = cast(BountyRepository, self.bountiesDB).serialize(**kwargs)
    ...
    def deserialize(cls, guildDict: SerializedBasedGuildUnion, dbReload=False, *, guildID: int, **kwargs) -> BasedGuild:
        ...
        if not bountiesDisabled:
            if "bountiesDB" in guildDict:
                ...
                bountiesDB = BountyRepository.deserialize(guildDict["bountiesDB"], # type: ignore[reportTypedDictNotRequiredAccess]
                                                    dbReload=dbReload, owningBasedGuild=newGuild)
            else:
                bountiesDB = BountyRepository(newGuild)
