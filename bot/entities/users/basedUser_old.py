...
from typing import Callable, NotRequired, Optional, Sequence, Tuple, Type, TypedDict, Union, TYPE_CHECKING, Dict, List, MutableSet, cast, TypeVar
...
from ...gameObjects.inventories import inventoryBase, userInventory
...
StoredInventoryUnion = Union[
    inventoryBase.Inventory[shipItem.Ship, shipBase.SerializedShipUnion],
    inventoryBase.Inventory[moduleItem.ModuleItem, moduleItem.SerializedModuleItemUnion],
    inventoryBase.Inventory[primaryWeapon.PrimaryWeapon, primaryWeapon.SerializedWeaponUnion],
    inventoryBase.Inventory[turretWeapon.TurretWeapon, turretWeapon.SerializedWeaponUnion],
    inventoryBase.Inventory[toolItem.ToolItem, toolItem.SerializedToolItemUnion],
]
...
{
    "credits": 0, "bountyCooldownEnd": 0.0, "lifetimeBountyCreditsWon": 0, "systemsChecked": 0, "bountyWins": 0,
...
UserHasMedal = Table(
    ...
    Column("medalId", ForeignKey("medal.id"), primary_key=True),
)


class BasedUser(Base, SerializesToSchema[SerializedBasedUser]):
    ...
    kaamo: Mapped[KaamoInventory] = relationship()
    loma: Mapped[LomaShop] = relationship()
    hangar: Mapped[UserHangarInventory] = relationship()
    activeShip: Mapped[shipItem.Ship] = relationship()
    medals: Mapped[MutableSet[Medal]] = relationship(secondary=UserHasMedal)
    duelRequests: Mapped[List[duelRequest.DuelRequest]] = relationship()
    ...
    def __init__(self,
        ...
        :param datetime bountyCooldownEnd: When the user's cmd_check cooldown is due to expire (Default now)
        ...
        :param UserHangarInventory: THe user's stored (unequipped) items (Default empty inventory)
        ...
        :param int alerts: One-hot integer flags indicating a user's subscriptions to user alerts where the state is managed by the bot
            (Default 0)
        :param int homeGuildID: The ID of this user's 'home guild' - the only guild from which they may use several
            commands e.g buy and check.
        :param datetime guildTransferCooldownEnd: A timestamp after which this user is allowed to transfer their homeGuildID.
    ...
    def getInventoryForItem(self, item: TItem) -> inventoryBase.Inventory[TItem, SerializedStoredItemUnion]:
        ...
        if isinstance(item, shipItem.Ship):
            return cast(inventoryBase.Inventory[TItem, shipBase.SerializedShipUnion], self.inactiveShips)
        elif isinstance(item, primaryWeapon.PrimaryWeapon):
            return cast(inventoryBase.Inventory[TItem, primaryWeapon.SerializedWeaponUnion], self.inactiveWeapons)
        elif isinstance(item, turretWeapon.TurretWeapon):
            return cast(inventoryBase.Inventory[TItem, turretWeapon.SerializedWeaponUnion], self.inactiveTurrets)
        elif isinstance(item, toolItem.ToolItem):
            return cast(inventoryBase.Inventory[TItem, toolItem.SerializedToolItemUnion], self.inactiveTools)
        elif isinstance(item, moduleItem.ModuleItem):
            return cast(inventoryBase.Inventory[TItem, moduleItem.SerializedModuleItemUnion], self.inactiveModules)
    ...
    def deserialize(cls, userDict: SerializedBasedUser, **kwargs) -> BasedUser:
        ...
        inactiveShips = inventoryBase.Inventory(shipItem.Ship)
        inactiveWeapons = inventoryBase.Inventory(primaryWeapon.PrimaryWeapon)
        inactiveModules = inventoryBase.Inventory(moduleItem.ModuleItem)
        inactiveTurrets = inventoryBase.Inventory(turretWeapon.TurretWeapon)
