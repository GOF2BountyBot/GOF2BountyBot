    ...
    for currentItemType in itemTypes:
        ...
        for itemNum in range(1, currentStock.numKeys + 1):
            ...
            shopEmbed.add_field(name=str(itemNum) + ". " \
                                    ...
                                    + "**" + currentItem.name + "** " + \
                                        ("PRESTIGED" if itemListing.userPrestiged else "NOT PRESTIGED"),
...
async def dev_cmd_kaamo_toggle_item_prestige(message : discord.Message, args : str, isDM : bool):
    """Toggle a requested user's item's prestiged status
    arg 1: user mention or ID
    arg 2: item type (ship/weapon/module/turret)
    arg 3: item number (from $debug-kaamo)
    ...
    if len(argsSplit) < 3:
        await message.channel.send(":x: Not enough arguments! Please provide a user, an item type " \
                                    + "(ship/weapon/module/turret/tool) and an item number from `" + prefix + "debug-kaamo`")
        return
    if len(argsSplit) > 3:
        await message.channel.send(":x: Too many arguments! Please only give a user, an item type " \
                                    + "(ship/weapon/module/turret/tool), and an item number.")
        return
    ...
    requestedListing = kaamoItemStock[itemNum - 1]
    ...
    requestedListing.userPrestiged = not requestedListing.userPrestiged

    await message.channel.send(f":white_check_mark: Set prestiged to {requestedListing.userPrestiged} on " \
                                + lib.discordUtil.userOrMemberName(requestedUser, message.guild) \
                                + "'s kaamo: " + itemName, embed=itemEmbed)
...
botCommands.register("kaamo-toggle-item-prestige", dev_cmd_kaamo_toggle_item_prestige, 3, allowDM=True, helpSection="kaamo", useDoc=True)
