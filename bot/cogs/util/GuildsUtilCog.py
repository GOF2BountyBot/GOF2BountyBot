from typing import Optional, Tuple, Callable, Any, cast, Coroutine, List
...
#region util
    ...
    async def getGuildWithBounties(self, interaction: Interaction, guild_id: str, sendError: bool = True, allowAllGuilds: bool = True) -> Tuple[bool, Optional[basedGuild.BasedGuild]]:
        """Gets a guild if one is specified. If not, then get the current guild.
        ...
        if guild_id in ["this", "here"]:
            if interaction.guild is None:
                if sendError:
                    await interaction.response.send_message("Please use this command from within a guild, or specify a guild.", ephemeral=True)
                return False, None
            callingBBGuild: Optional[basedGuild.BasedGuild] = self.bot.guildsDB.getGuild(interaction.guild.id)
        elif guild_id == "all":
            if not allowAllGuilds:
                if sendError:
                    await interaction.response.send_message("This command cannot be applied to all guilds.", ephemeral=True)
                return False, None
            callingBBGuild = None
        elif not lib.stringTyping.isInt(guild_id):
            if sendError:
                await interaction.response.send_message(":x: Invalid guild id - not a number. Please give an ID, `all` or `this`.", ephemeral=True)
            return False, None
        else:
            guildID = int(guild_id)
            if not self.bot.guildsDB.idExists(guildID):
                if sendError:
                    await interaction.response.send_message(f"Unrecognised guild ID: {guildID}", ephemeral=True)
                return False, None
            callingBBGuild = self.bot.guildsDB.getGuild(guildID)

        if callingBBGuild is not None and callingBBGuild.bountiesDisabled:
            if sendError:
                await interaction.response.send_message(f":x: Bounties are disabled in that guild! ('{'Unknown' if callingBBGuild.dcGuild is None else callingBBGuild.dcGuild.name}')", ephemeral=True)
            return False, None
