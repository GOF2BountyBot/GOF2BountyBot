...
class DevKaamoCog(basedApp.BasedCog):
    ...
    async def dev_cmd_debug_kaamo(self, interaction: Interaction, user_id: str = ""):
        ...
        for itemType in itemTypes:
            ...
            if currentStock.numKeys != expectedNumKeys:
                ...
                self.bot.logger.log("dev_kaamo", "dev_cmd_debug_kaamo",
            ...
            for itemNum, currentKey in enumerate(currentStock.keys):
                ...
                if not isinstance(itemListing, DiscountableItemListing):
                    ...
                    currentItemDiscountsStr = "⚠ Non-discountable listing type"
                else:
                    currentItemDiscountsStr = ", ".join(f"'{discount.desc}': {discount.mult}" for discount in itemListing.discounts) \
                                                if len(itemListing.discounts) > 0 else "No discounts."
                ...
                shopEmbed.add_field(name=f"{itemNum+1}. {currentitemEmoji} {currentItemCountStr} **{currentItem.name}**",
                                    value=f"{currentItem.statsStringShort()}\n*{currentItemDiscountsStr}*", inline=True)
    ...
    async def dev_cmd_del_kaamo_item(self, interaction: Interaction, item_type: ItemCategory, item_number: Range[int, 1, ...], user_id: str = ""):
        ...
        if item_number > kaamoItemStock.numKeys:
            await interaction.response.send_message(f":x: Invalid item number! The user only has {kaamoItemStock.numKeys} {item_type.value}s.")
            return
        if item_number < 1:
            await interaction.response.send_message(":x: Invalid item number! Must be at least 1.")
            return
